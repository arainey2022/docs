---
title: "AI Actions (Tools)"
description: "Having your AI support agent answer questions based on knowledge and user data is all well and good, but what about if you want it to start doing things for you, on your behalf, to truly automate?"
---
...
The [User Data API](/api-documentation/user-data-api) is to be used when you need to retrieve information or data about a specific user and can only be used where you have a verified user (email support ticket or authenticated live chat user).

What API request methods can Actions (Tools) APIs support?

Actions (Tools) can support any API request method, including:
...
- **API endpoint:** e.g. `https://api.company.com/orders`
...
### How many characters can be included in a tool call response?

The maximum response size (in characters) for a tool call response is 15,000 characters. If responses exceed this size then the response will be trimmed to the first 15,000 characters before reaching the agent.