...
4. If you have multiple Zendesk accounts, you will need to input your Zendesk subdomain, which you can find by looking at the URL of your Admin Center or Agent Dashboard e.g. (the text in **bold below** is the subdomain):\
   \
           _https://**teammyaskai**.zendesk.com_\
...
10. After tickets have been 'Solved' they need to be moved to 'Closed', so that the AI agent can take control of the conversation again. There are two ways you can do this.
    1. Go to **_Objects and rules \> Automations_**, and find the existing automation called "Close ticket 4 days". Edit the "Hours since status..." field to 1 (recommended, but you can choose your own value). This means after tickets are solved, they will be closed within an hour and control returned to the AI assistant.\
       <Frame>
         <img
    ...
    2. If you want to immediately pass back control to the AI agent when a ticket has been solved, then you will need to set up a 'Trigger'. To do this, go to **_Objects and rules \> Triggers \> Create trigger._**\
       \
...
<Warning>
  ...
  \
  You can, however, pause replies at any time once set up by going to **Channels \> Zendesk \> Zendesk Messaging**, scrolling to the bottom of the page and toggling "Pause AI agent" on.
</Warning>
